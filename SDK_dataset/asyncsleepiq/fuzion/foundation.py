...
class SleepIQFuzionFoundation(SleepIQFoundation):
    ...
    async def init_foot_warmers(self) -> None:
        ...
        for side in [Side.LEFT, Side.RIGHT]:
            result = self._api.bamkey(self.bed_id, "GetFootwarmingPresence", args=[SIDES_FULL[side].lower()])
            if result == "1":
    ...
    async def init_core_climates(self) -> None:
        ...
        for side in [Side.LEFT, Side.RIGHT]:
            result = self._api.bamkey(self.bed_id, "GetHeidiPresence", args=[SIDES_FULL[side].lower()])
            if result == "true":
    ...
    async def fetch_features(self) -> None:
        ...
        vals = self._api.bamkey(self.bed_id, "GetSystemConfiguration")
    ...
    async def stop_motion(self, side: str) -> None:
        ...
        self._api.bamkey(self.bed_id, "HaltAllActuators")
