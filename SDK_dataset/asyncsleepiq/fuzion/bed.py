...
class SleepIQFuzionBed(SleepIQBed):
    ...
    async def valid(self) -> bool:
        try:
            self._api.bamkey(self.id, "GetSleepiqPrivacyState")
            return True
        except:
            return False
    ...
    async def stop_pump(self) -> None:
        """Stop pump."""
        self._api.bamkey(self.id, "InterruptSleepNumberAdjustment")

    async def fetch_pause_mode(self) -> None:
        """Update paused attribute with data from API."""
        status = self._api.bamkey(self.id, "GetSleepiqPrivacyState")
    ...
    async def set_pause_mode(self, mode: bool) -> None:
        ...
        self._api.bamkey(self.id, "SetSleepiqPrivacyState", args=[arg])
        self.paused = mode
