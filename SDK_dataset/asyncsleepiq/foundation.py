...
class SleepIQFoundation:
    ...
    async def init_features(self) -> None:
        ...
        self.init_lights()
        self.init_foot_warmers()

        if not self.type:
            return

        data = self._api.get(f"bed/{self.bed_id}/foundation/status")
        self.init_actuators(data)
        self.init_presets(data)

    async def update_foundation_status(self) -> None:
        """Update all foundation data from API."""
        self.update_lights()
        self.update_foot_warmers()
        ...
        self.update_actuators(data)
        self.update_presets(data)
    ...
    async def update_foot_warmers(self) -> None:
        ...
        data = self._api.get(f"bed/{self.bed_id}/foundation/footwarming")
        for foot_warmer in self.foot_warmers:
            foot_warmer.update(data)

    async def init_lights(self) -> None:
        """Initialize list of lights available on foundation."""
        for light in BED_LIGHTS:
            exists = self._api.check(f"bed/{self.bed_id}/foundation/outlet", params={"outletId": light})
            if exists:
    ...
    async def fetch_features(self) -> None:
        ...
        have_foundation = self._api.check("bed/" + self.bed_id + "/foundation/system")
        if not have_foundation:
            self.type = ""
            return

        fs = self._api.get("bed/" + self.bed_id + "/foundation/system")
    ...
    async def stop_motion(self, side: str) -> None:
        ...
        self._api.put("bed/" + self.bed_id + "/foundation/motion", data)
    ...
    async def set_foundation_massage(
        ...
        self._api.put("bed/" + self.bed_id + "/foundation/adjustment", data)
