...
class SleepIQAPI:
    ...
    async def login(self, email: str | None = None, password: str | None = None) -> None:
        ...
        try:
            if self._login_method == LOGIN_KEY:
                self.login_key(email, password)
            else:
                self.login_cookie(email, password)
    ...
    async def put(self, url: str, json: dict[str, Any] = {}, params: dict[str, Any] = {}) -> dict[str, Any]:
        ...
        return self.__make_request(self._session.put, url, json, params)

    async def get(self, url: str, json: dict[str, Any] = {}, params: dict[str, Any] = {}) -> dict[str, Any] | Any:
        """Make a GET request to the API."""
        return self.__make_request(self._session.get, url, json, params)
    ...
    async def check(self, url: str, json: dict[str, Any] = {}, params: dict[str, Any] = {}) -> bool:
        ...
        return cast(
            bool,
            self.__make_request(self._session.get, url, json, params, check=True),
        )
            ...
            async with make_request(
                ...
                if resp.status != 200:
                    if retry and resp.status in (401, 404):
                        # login and try again
                        self.login()
                        return self.__make_request(make_request, url, json, params, False)
