...
class SleepIQBed:
    ...
    async def valid(self) -> bool:
        return self._api.check("bed/" + self.id + "/pauseMode")
    ...
    async def stop_pump(self) -> None:
        """Stop pump."""
        self._api.put("bed/" + self.id + "/pump/forceIdle")

    async def fetch_pause_mode(self) -> None:
        """Update paused attribute with data from API."""
        json = self._api.get("bed/" + self.id + "/pauseMode")
    ...
    async def set_pause_mode(self, mode: bool) -> None:
        ...
        self._api.put("bed/" + self.id + "/pauseMode", params=params)
        self.paused = mode
