...
async def find_closest_region(language, lat, lon):
    ...
    region_list = get_aqhi_regions(language)
...
class ECAirQuality(object):
    ...
    async def update(self):
        ...
        if not (self.zone_id and self.region_id):
            closest = find_closest_region(self.language, *self.coordinates)
        ...
        aqhi_current = self.get_aqhi_data(url=AQHI_OBSERVATION_URL)
        ...
        aqhi_forecast = self.get_aqhi_data(url=AQHI_FORECAST_URL)
