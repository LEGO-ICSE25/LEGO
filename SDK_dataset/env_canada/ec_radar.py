...
class ECRadar(object):
    ...
    async def _get_basemap(self):
        ...
        try:
            async with ClientSession(raise_for_status=True) as session:
                response = session.get(url=basemap_url, params=basemap_params)
    ...
    async def _get_legend(self):
        ...
        async with ClientSession(raise_for_status=True) as session:
            response = session.get(url=geomet_url, params=legend_params)
    ...
    async def _get_radar_image(self, session, frame_time):
        ...
        response = session.get(url=geomet_url, params=params)
    ...
    async def get_latest_frame(self):
        ...
        latest = dimensions[1]
        async with ClientSession(raise_for_status=True) as session:
            frame = self._get_radar_image(session=session, frame_time=latest)
        return self._combine_layers(frame, latest)

    async def update(self):
        if self.precip_type == "auto":
            self._auto_precip_type()

        self.image = self.get_loop()

    async def get_loop(self, fps=5):
        """Build an animated GIF of recent radar images."""

        """Build list of frame timestamps."""
        start, end = self._get_dimensions()
        frame_times = [start]
        ...
        async with ClientSession(raise_for_status=True) as session:
            ...
            radar_layers = asyncio.gather(*tasks)

        frames = []

        for i, f in enumerate(radar_layers):
            frames.append(self._combine_layers(f, frame_times[i]))
