...
class PhilipsTV(object):
    ...
    async def getReq(self, path, protocol = None) -> Optional[Dict]:
        try:
            resp = self.session.get(self._url(path, protocol = protocol))
    ...
    async def postReq(self, path: str, data: Any, timeout=None, protocol=None) -> Optional[Dict]:
        try:
            resp = self.session.post(self._url(path, protocol), json=data, timeout=timeout)
            if resp.status_code == 401:
                raise AutenticationFailure("Authenticaion failed to device")
    ...
    async def update(self):
        try:
            if not self.on:
                ...
                self.getAmbilightSupportedStyles()
                self.getAmbilightCached()
                self.getMenuItemsSettingsStructure()
            ...
            self.getContext()
            self.getScreenState()
            self.getAmbilightMode()
            self.getAmbilightPower()
            self.getAmbilightCurrentConfiguration()
            self.getHueLampPower()
            self.getRecordings()
            self.on = True
            return True
    ...
    async def getSystem(self):
        ...
        for prot in protocols:
            r = cast(Optional[SystemType], self.getReq("system", protocol=prot))
            if r:
    ...
    async def getChannels(self):
        if self.api_version >= 5:
            ...
            r = cast(Optional[ChannelsType], self.getReq("channels"))
            if r:
                self.channels = r
            else:
                self.channels = {}
    ...
    async def getChannelId(self):
        ...
        if self.api_version >= 5:
            r = cast(Optional[ActivitesTVType], self.getReq("activities/tv"))
        else:
            r = cast(
                Optional[ChannelsCurrentType], self.getReq("channels/current")
            )
    ...
    async def getChannelLogo(
        ...
        if self.api_version >= 5:
            data, content_type = self._getBinary(
    ...
    async def getContext(self) -> Optional[ContextType]:
        r = cast(Optional[ContextType], self.getReq(f"context"))
        self.context = r
        return r
    ...
    async def setChannel(self, ccid: Union[str, int], list_id: Optional[str] = None):
        ...
        if self.api_version >= 5:
            ...
            if self.postReq("activities/tv", cast(Dict, channel)) is not None:
                self.channel = channel
                return True
        else:
            channel = {"id": str(ccid)}
            if self.postReq("channels/current", cast(Dict, channel)) is not None:
                self.channel = channel
                return True
        return False
    ...
    async def getChannelLists(self):
        if self.api_version >= 5:
            r = cast(ChannelDbTv, self.getReq("channeldb/tv"))
            if r:
                channel_lists = {}
                favorite_lists = {}
                ...
                for data in r.get("channelLists", []):
                    ...
                    channel_list = cast(
                        ...
                        self.getReq(f"channeldb/tv/channelLists/{list_id}"),
                    )
                    if channel_list:
                ...
                for data in r.get("favoriteLists", []):
                    ...
                    favorite_list = cast(
                        ...
                        self.getReq(f"channeldb/tv/favoriteLists/{list_id}"),
                    )
                    if favorite_list:
    ...
    async def setSource(self, source_id):
        if self.api_version == 1:
            r = self.postReq("sources/current", {"id": source_id})
            if r is not None:
                return True
            return False
        ...
        if self.json_feature_supported("activities", "intent"):
            ...
            return self.setApplication(intent)

        return False

    async def getApplications(self):
        if self.json_feature_supported("applications"):
            r = cast(ApplicationsType, self.getReq("applications"))
            if r:
    ...
    async def getApplication(self):
        if self.json_feature_supported("applications"):
            r = cast(ApplicationIntentType, self.getReq("activities/current"))
            if r:
                self.application = r
            else:
    ...
    async def setPowerState(self, state):
        ...
        if self.postReq("powerstate", data) is not None:
    ...
    async def setScreenState(self, state):
        if self.api_version >= 5:
            ...
            if self.postReq("screenstate", data) is not None:
    ...
    async def setHueLampPower(self, state):
        if self.json_feature_supported("ambilight", "Hue"):
            data = {"power": state}
            if self.postReq("HueLamp/power", data) is not None:
    ...
    async def setApplication(self, intent: ApplicationIntentType):
        if self.json_feature_supported("activities", "intent"):
            data = {"intent": intent}
            if self.postReq("activities/launch", data) is not None:
    ...
    async def setVolume(self, level, muted=False):
        ...
        if self.postReq("audio/volume", data) is None:
            return False

        self.audio_volume.update(data)

    async def sendKey(self, key):
        res = self.postReq("input/key", {"key": key}) is not None
        if res:
            if key == "Mute":
                if self.audio_volume:
    ...
    async def sendUnicode(self, key: str):
        return self.postReq("input/key", {"unicode": key}) is not None

    async def getAmbilightMode(self):
        data = self.getReq("ambilight/mode")
        if data:
    ...
    async def setAmbilightMode(self, mode):
        data = {"current": mode}
        if self.postReq("ambilight/mode", data) is None:
            return False
    ...
    async def getAmbilightTopology(self):
        r = self.getReq("ambilight/topology")
        if r:
    ...
    async def getAmbilightMeasured(self):
        r = self.getReq("ambilight/measured")
        if r:
    ...
    async def getAmbilightProcessed(self):
        r = self.getReq("ambilight/processed")
        if r:
    ...
    async def getAmbilightCached(self):
        ...
        sparingly.
        """
        r = self.getReq("ambilight/cached")
        if r:
    ...
    async def setAmbilightPower(self, power: str):
        if self.api_version >= 5:
            data = {"power": power}
            if self.postReq("ambilight/power", data) is None:
                return False
        ...
        if self.api_version == 1:
            if power == "On":
                self.setAmbilightMode("internal")
            else:
                self.setAmbilightMode("manual")
                self.setAmbilightCached({"r": 0, "g": 0, "b": 0})

            return True

    async def setAmbilightCached(self, data):
        if self.postReq("ambilight/cached", data) is None:
            return False
    ...
    async def getAmbilightSupportedStyles(self):
        if self.json_feature_supported("ambilight", "Ambilight"):
            r = cast(
                ...
                self.getReq("ambilight/supportedstyles"),
            )
            if r:
    ...
    async def getAmbilightCurrentConfiguration(self):
        if self.json_feature_supported("ambilight", "Ambilight"):
            r = cast(
                ...
                self.getReq("ambilight/currentconfiguration"),
            )
            if r:
    ...
    async def setAmbilightCurrentConfiguration(
        ...
        if self.json_feature_supported("ambilight", "Ambilight"):
            r = self.postReq("ambilight/currentconfiguration", cast(Dict, config))
            if r is None:
                self.ambilight_current_configuration = None
                return False
    ...
    async def openURL(self, url: str):
        if self.json_feature_supported("activities", "browser"):
            r = self.postReq("activities/browser", {"url": url})
    ...
    async def getStringsCached(self, string_ids: Iterable[str]) -> Union[Dict[str, str], None]:
        ...
        if string_ids_missing:
            if (data := self.getStrings(sorted(string_ids_missing))) is None:
                return None
    ...
    async def getStrings(
        ...
        for group in chunked_iterator(MAXIMUM_ITEMS_IN_REQUEST, strings):
            data = self._getStrings(group, language=language, country=country, variant=variant)
            res.update(data)
        return res
    ...
    async def _getStrings(
        ...
        res = cast(Optional[Strings], self.postReq("strings", data))
        if res:
            return {
    ...
    async def getMenuItemsSettingsStructure(self, force=False) -> Optional[MenuItemsSettingsStructure]:
        if self.json_feature_supported("menuitems", "Setup_Menu") or force:
            r = cast(Optional[MenuItemsSettingsStructure],
                    self.getReq("menuitems/settings/structure")
            )
            self.settings = r
            return r
    ...
    async def getMenuItemsSettingsCurrent(self, node_ids: Iterable[int], force=False) -> Optional[MenuItemsSettingsCurrent]:
        ...
        for group in chunked_iterator(MAXIMUM_ITEMS_IN_REQUEST, node_ids):
            if (data := self._getMenuItemsSettingsCurrent(group)) is None:
                return None
    ...
    async def _getMenuItemsSettingsCurrent(self, node_ids: Iterable[int], force=False) -> Optional[MenuItemsSettingsCurrent]:
        ...
        if self.json_feature_supported("menuitems", "Setup_Menu") or force:
            ...
            r = cast(Optional[MenuItemsSettingsCurrent],
                     self.postReq("menuitems/settings/current", cast(dict, post))
            )
            return r
    ...
    async def getMenuItemsSettingsCurrentValue(self, node_ids: List[int], force=False) -> Dict[int, Optional[MenuItemsSettingsCurrentValueValue]]:
            data = self.getMenuItemsSettingsCurrent(node_ids, force=force)
    ...
    async def postMenuItemsSettingsUpdate(self, post: MenuItemsSettingsUpdate, force=False):
        if self.json_feature_supported("menuitems", "Setup_Menu") or force:
            return self.postReq("menuitems/settings/update", cast(dict, post))
    ...
    async def postMenuItemsSettingsUpdateData(self, value: Dict[int, MenuItemsSettingsValueData], force=False):
        ...
        return self.postMenuItemsSettingsUpdate(cast(MenuItemsSettingsUpdate, post), force=force)
    ...
    async def notifyChange(self, timeout=TIMEOUT_NOTIFYREAD):
        ...
        try:
            result = self.postReq("notifychange", data=data, timeout=timeout_ctx)
