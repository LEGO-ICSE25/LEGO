...
class AsyncBulb(Bulb):
    ...
    async def async_send_command(self, method, params):
        ...
        async with command_lock:
            if self._music_mode_state:
                ...
                return {"result": ["ok"]}
            future = self._async_send_command(method, params)
    ...
    async def _async_reconnect_loop(self):
        ...
        while self._is_listening:
            try:
                async with asyncio_timeout(TIMEOUT):
                    reader, writer = asyncio.open_connection(self._ip, self._port)
                asyncio.sleep(0.1)
        ...
        while self._is_listening:
            try:
                ...
                async with asyncio_timeout(PING_INTERVAL + TIMEOUT):
                    line = self._async_reader.readline()
                ...
                if self._music_mode_state:
                    ...
                    if self._last_properties["power"] == "on":
                        ...
                        self.async_turn_off(is_ping=True)
                    continue
                timeouts += 1
                if timeouts == 2:
                ...
                ping_id = self._async_send_command(
    ...
    async def async_get_properties(
        ...
        response = self.async_send_command("get_prop", requested_properties)
    ...
    async def async_set_power_mode(self, mode):
        ...
        """
        return self.async_turn_on(power_mode=mode)
    ...
    async def async_start_music(self, port=0, ip=None, reconnect=False):
        ...
        async with self._music_mode_lock:
            ...
            if reconnect:
                ...
                self.async_get_properties(when_on=True)
            else:
            ...
            async with self._async_command_lock:
                self.async_send_command("set_music", [1, local_ip, port])
                self.async_stop_listening(False)
                try:
                    async with asyncio_timeout(0.5):
                        reader, writer = asyncio.shield(future)
                except asyncio.TimeoutError:
                    ...
                    try:
                        async with asyncio_timeout(TIMEOUT - 0.5):
                            reader, writer = future
                    except asyncio.TimeoutError as ex:
                        # Ensures a full reconnect to the bulb
                        self.async_stop_music(force=True)
                        raise BulbException(
    ...
    async def async_stop_music(self, force=False, **kwargs):
        ...
        self.async_listen(self._async_callback)
