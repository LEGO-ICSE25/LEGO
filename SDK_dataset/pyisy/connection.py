...
class Connection:
    ...
    async def test_connection(self):
        ...
        config = self.get_config(retries=None)
        if not config:
    ...
    async def request(self, url, retries=0, ok404=False, delay=0):
        ...
        try:
            async with self.semaphore, self.req_session.get(
                ...
                if res.status == HTTP_OK:
                    ...
                    results = res.text(encoding="utf-8", errors="ignore")
        ...
        if retries < MAX_RETRIES:
            ...
            asyncio.sleep(RETRY_BACKOFF[retries])
            # recurse to try again
            retry_result = self.request(url, retries + 1, ok404=ok404)
            return retry_result
        # fail for good
        _LOGGER.error(
    ...
    async def get_config(self, retries=0):
        ...
        result = self.request(req_url, retries=retries)
        return result
