...
class BaseManager:
    ...
    async def set_synchronization_point(self) -> float:
        ...
        try:
            self._service.SetSynchronizationPoint()
    ...
    async def _renew_subscription(self) -> float | None:
        ...
        if self.closed() or self._shutdown:
            return None
        try:
            return self._call_subscription_renew()
    ...
    async def _renew_or_restart_subscription(self) -> None:
        ...
        try:
            renewal_call_at = (
                self._renew_subscription() or self._restart_subscription()
            )
        finally:
...
class NotificationManager(BaseManager):
    ...
    async def _start(self) -> float:
        ...
        notify_service = device.create_notification_service()
        time_str = device.get_next_termination_time(self._interval)
        result = notify_service.Subscribe(
            {
        ...
        self._subscription = device.create_subscription_service(
        ...
        if device.has_broken_relative_time(
            ...
            result = self._subscription.Renew(
                device.get_next_termination_time(self._interval)
            )
...
class PullPointManager(BaseManager):
    ...
    async def _start(self) -> float:
        ...
        events_service = device.create_events_service()
        result = events_service.CreatePullPointSubscription(
            {
        ...
        self._service = device.create_pullpoint_service()
        if device.has_broken_relative_time(
