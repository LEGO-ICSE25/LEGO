...
async def _cached_document(url: str) -> Document:
    ...
    document = loop.run_in_executor(None, _load_document)
...
class ONVIFService:
    ...
    async def setup(self):
        ...
        self.document = _cached_document(self.url)
...
class ONVIFCamera:
    ...
    async def update_xaddrs(self):
        ...
        if self.adjust_time:
            try:
                sys_date = devicemgmt.authless_GetSystemDateAndTime()
            except zeep.exceptions.Fault:
                # Looks like we should try with auth
                sys_date = devicemgmt.GetSystemDateAndTime()
    ...
    async def get_snapshot_uri(self, profile_token: str) -> str:
        ...
        if uri is _SENTINEL:
            ...
            uri = None
            try:
                result = media_service.GetSnapshotUri(req)
    ...
    async def get_snapshot(
        ...
        uri = self.get_snapshot_uri(profile_token)
        if uri is None:
            return None
        ...
        try:
            response = self._snapshot_client.get(uri, auth=auth)
