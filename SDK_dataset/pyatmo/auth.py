...
class AbstractAsyncAuth(ABC):
    ...
    async def async_post_api_request(
        ...
        return self.async_post_request(
            url=(base_url or self.base_url) + endpoint,
            params=params,
            timeout=timeout,
        )
    ...
    async def process_response(self, resp, url):
        ...
        if not resp.ok:
            ...
            self.handle_error_response(resp, resp_status, url)

        return self.handle_success_response(resp, resp_content)
    ...
    async def async_addwebhook(self, webhook_url: str) -> None:
        """Register webhook."""
        try:
            resp = self.async_post_api_request(
