...
class YoLinkHome:
    ...
    async def async_setup(
        ...
        home_info: BRDP = self.async_get_home_info()
        # load home devices
        self.async_load_home_devices()
    ...
    async def async_load_home_devices(self, **kwargs: Any) -> dict[str, YoLinkDevice]:
        """Get home devices."""
        response: BRDP = self._http_client.execute(
        ...
        for _device in response.data["devices"]:
            ...
            if _yl_device.device_type in has_external_data_devices:
                try:
                    dev_external_data_resp = _yl_device.get_external_data()
