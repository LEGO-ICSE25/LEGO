...
async def motion_detection(ctx, ring: Ring, device_name, turn_on, turn_off):
    ...
    if not device:
        ...
        return ctx.invoke(list_command)
...
async def ainput(string: str) -> str:
    ...
    loop.run_in_executor(None, lambda s=string: sys.stdout.write(s + " "))
    ...
    while loop.is_running() and not line:
        line = loop.run_in_executor(None, functools.partial(read_with_timeout, 1))
    return line
...
async def listen(
    ...
    ainput("Listening, press enter to cancel\n")
