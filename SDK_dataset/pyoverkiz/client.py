...
class OverkizClient:
    ...
    async def login(
        ...
        if self.server == SUPPORTED_SERVERS[Server.SOMFY_EUROPE]:
            self.somfy_tahoma_get_access_token()
    ...
    async def nexity_login(self) -> str:
        ...
        # Request access token
        client = loop.run_in_executor(None, _get_client)
        ...
        try:
            tokens = loop.run_in_executor(None, aws.authenticate_user)
    ...
    async def get_gateways(self, refresh: bool = False) -> list[Gateway]:
        ...
        response = self.__get("setup/gateways")
    ...
    async def get_execution_history(self) -> list[HistoryExecution]:
        ...
        """
        response = self.__get("history/executions")
    ...
    async def refresh_states(self) -> None:
        ...
        """
        self.__post("setup/devices/states/refresh")
    ...
    async def register_event_listener(self) -> str:
        ...
        """
        response = self.__post("events/register")
    ...
    async def get_current_execution(self, exec_id: str) -> Execution:
        ...
        response = self.__get(f"exec/current/{exec_id}")
    ...
    async def get_current_executions(self) -> list[Execution]:
        ...
        response = self.__get("exec/current")
    ...
    async def get_api_version(self) -> str:
        ...
        response = self.__get("apiVersion")
    ...
    async def execute_command(
        ...
        response: str = self.execute_commands(device_url, [command], label)
    ...
    async def cancel_command(self, exec_id: str) -> None:
        ...
        self.__delete(f"/exec/current/setup/{exec_id}")
    ...
    async def execute_commands(
        ...
        response: dict = self.__post("exec/apply", payload)
    ...
    async def get_scenarios(self) -> list[Scenario]:
        """List the scenarios"""
        response = self.__get("actionGroups")
    ...
    async def generate_local_token(self, gateway_id: str) -> str:
        ...
        """
        response = self.__get(f"config/{gateway_id}/local/tokens/generate")
    ...
    async def get_local_tokens(
        ...
        Access scope : Full enduser API access (enduser/*)
        """
        response = self.__get(f"config/{gateway_id}/local/tokens/{scope}")
    ...
    async def delete_local_token(self, gateway_id: str, uuid: str) -> bool:
        ...
        Access scope : Full enduser API access (enduser/*)
        """
        self.__delete(f"config/{gateway_id}/local/tokens/{uuid}")
    ...
    async def execute_scenario(self, oid: str) -> str:
        """Execute a scenario"""
        response = self.__post(f"exec/{oid}")
        return cast(str, response["execId"])
    ...
    async def execute_scheduled_scenario(self, oid: str, timestamp: int) -> str:
        ...
        response = self.__post(f"exec/schedule/{oid}/{timestamp}")
    ...
    async def get_setup_options(self) -> list[Option]:
        ...
        Access scope : Full enduser API access (enduser/*)
        """
        response = self.__get("setup/options")
    ...
    async def get_setup_option(self, option: str) -> Option | None:
        ...
        """
        response = self.__get(f"setup/options/{option}")
    ...
    async def get_setup_option_parameter(
        ...
        """
        response = self.__get(f"setup/options/{option}/{parameter}")
