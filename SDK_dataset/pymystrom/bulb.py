...
from __init__ import _request as request
...
class MyStromBulb:
    ...
    async def get_state(self) -> object:
        ...
        response = request(self, uri=self.uri)
        ...
        self._state = response[self._mac]["on"]
    ...
    async def set_color_hex(self, value):
        ...
        response = request(self, uri=self.uri, method="POST", data=data)
        return response
    ...
    async def set_sunrise(self, duration):
        ...
        """
        max_brightness = 100
        self.set_transition_time((duration / max_brightness))
        for i in range(0, duration):
            data = "action=on&color=3;{}".format(i)
            request(self, uri=self.uri, method="POST", data=data)
            asyncio.sleep(duration / max_brightness)
