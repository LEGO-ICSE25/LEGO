...
async def join(dev: SmartDevice, ssid, password, keytype):
    ...
    res = dev.wifi_join(ssid, password, keytype=keytype)
    echo(
...
async def find_host_from_alias(alias, target="255.255.255.255", timeout=1, attempts=3):
    ...
    for _attempt in range(1, attempts):
        found_devs = Discover.discover(target=target, timeout=timeout)
...
async def emeter(dev: SmartDevice, year, month, erase):
    ...
    if erase:
        ...
        return dev.erase_emeter_stats()

    if year:
        echo(f"== For year {year.year} ==")
        echo("Month, usage (kWh)")
        usage_data = dev.get_emeter_monthly(year=year.year)
    elif month:
        echo(f"== For month {month.month} of {month.year} ==")
        echo("Day, usage (kWh)")
        usage_data = dev.get_emeter_daily(year=month.year, month=month.month)
    else:
...
async def usage(dev: SmartDevice, year, month, erase):
    ...
    if erase:
        ...
        return usage.erase_stats()

    if year:
        echo(f"== For year {year.year} ==")
        echo("Month, usage (minutes)")
        usage_data = usage.get_monthstat(year=year.year)
    elif month:
        echo(f"== For month {month.month} of {month.year} ==")
        echo("Day, usage (minutes)")
        usage_data = usage.get_daystat(year=month.year, month=month.month)
    else:
        # Call with no argument outputs summary data and returns
...
async def brightness(dev: SmartBulb, brightness: int, transition: int):
    ...
    if brightness is None:
        ...
        return dev.set_brightness(brightness, transition=transition)
...
async def temperature(dev: SmartBulb, temperature: int, transition: int):
    ...
    if temperature is None:
        ...
        return dev.set_color_temp(temperature, transition=transition)
...
async def hsv(dev, ctx, h, s, v, transition):
    ...
    if h is None or s is None or v is None:
        ...
        return dev.set_hsv(h, s, v, transition=transition)
...
async def on(dev: SmartDevice, index: int, name: str, transition: int):
    ...
    return dev.turn_on(transition=transition)
...
async def off(dev: SmartDevice, index: int, name: str, transition: int):
    ...
    return dev.turn_off(transition=transition)
...
async def delete_rule(dev, id):
    ...
    if rule_to_delete:
        ...
        return schedule.delete_rule(rule_to_delete)
    else:
...
async def presets(ctx):
    ...
    if ctx.invoked_subcommand is None:
        return ctx.invoke(presets_list)
...
async def turn_on_behavior(dev: SmartBulb, type, last, preset):
    ...
    settings = dev.get_turn_on_behavior()
    ...
    return dev.set_turn_on_behavior(settings)
