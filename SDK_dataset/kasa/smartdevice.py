        ...
        async def wrapped(*args, **kwargs):
            ...
            if self._last_update is None:
                raise SmartDeviceException(
                    "You need to update() to access the data"
                )
...
class SmartDevice:
    ...
    * :class:`SmartLightStrip`
    ...
    To initialize, you have to :func:`update()` at least once.
    This will allow accessing the properties using the exposed properties.

    All changes to the device are done using awaitable methods,
    which will not change the cached values, but you must update() separately.
    ...
    async def _query_helper(
        ...
        try:
            response = self.protocol.query(request=request)
    ...
    async def update(self, update_children: bool = True):
        ...
        if self._last_update is None:
            ...
            response = self.protocol.query(req)
            self._last_update = response
            self._set_sys_info(response["system"]["get_sysinfo"])

        self._modular_update(req)
    ...
    async def _modular_update(self, req: dict) -> None:
        ...
        responses = [
            self.protocol.query(request) for request in request_list if request
        ]
    ...
    async def set_alias(self, alias: str) -> None:
        ...
        return self._query_helper("system", "set_dev_alias", {"alias": alias})
    ...
    async def get_time(self) -> Optional[datetime]:
        ...
        return self.modules["time"].get_time()
    ...
    async def get_timezone(self) -> Dict:
        ...
        return self.modules["time"].get_timezone()
    ...
    async def get_emeter_realtime(self) -> EmeterStatus:
        ...
        self._verify_emeter()
        return EmeterStatus(self.modules["emeter"].get_realtime())
    ...
    async def get_emeter_daily(
        ...
        """
        self._verify_emeter()
        return self.modules["emeter"].get_daystat(year=year, month=month, kwh=kwh)
    ...
    async def get_emeter_monthly(
        ...
        """
        self._verify_emeter()
        return self.modules["emeter"].get_monthstat(year=year, kwh=kwh)

    @requires_update
    async def erase_emeter_stats(self) -> Dict:
        """Erase energy meter statistics."""
        self._verify_emeter()
        return self.modules["emeter"].erase_stats()
    ...
    async def wifi_scan(self) -> List[WifiNetwork]:  # noqa: D202
        ...
        async def _scan(target):
            return self._query_helper(target, "get_scaninfo", {"refresh": 1})
        ...
        try:
            ...
            info = _scan("smartlife.iot.common.softaponboarding")
    ...
    async def wifi_join(self, ssid, password, keytype=3):  # noqa: D202
        ...
        async def _join(target, payload):
            return self._query_helper(target, "set_stainfo", payload)

        payload = {"ssid": ssid, "password": password, "key_type": keytype}
        try:
            return _join("netif", payload)
        except SmartDeviceException as ex:
            _LOGGER.debug(
                "Unable to join using 'netif', retrying with 'softaponboarding': %s", ex
            )
            return _join("smartlife.iot.common.softaponboarding", payload)
