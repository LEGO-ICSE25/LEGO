...
class Broker:
    ...
    async def populate_user_data(self) -> _UserDataT:
        ...
        user_data, _ = self._populate_user_data()
        return user_data
    ...
    async def _populate_user_data(self) -> tuple[_UserDataT, aiohttp.ClientResponse]:
        ...
        url = "/session"
        response = self.make_request(HTTPMethod.POST, url, data=self._POST_DATA)

        self._user_data: _UserDataT = response.json()
    ...
    async def populate_full_data(self) -> list[_LocnDataT]:
        ...
        response = self.make_request(HTTPMethod.GET, url, data=self._POST_DATA)

        self._full_data: list[_LocnDataT] = response.json()
    ...
    async def _make_request(
        ...
        *,
        ...
        async with func(url, json=data, headers=self._headers) as response:  # NB: json=
            response_text = response.text()  # why cant I move this below the if?
            ...
            _, response = self._populate_user_data()  # Get a fresh sessionId
    ...
    async def make_request(
        ...
        *,
        ...
        try:
            response = self._make_request(func, url, data=data)
