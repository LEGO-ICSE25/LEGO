...
class Host:
    ...
    async def login(self) -> None:
        ...
        try:
            ...
            self.logout(login_mutex_owned=True)  # Ensure there would be no "max session" error
            ...
            try:
                json_data = self.send(body, param, expected_response_type="json")
        ...
        try:
            if self._token:
                ...
                try:
                    ...
                    self.send(body, param, expected_response_type="text/html")
    ...
    async def get_state(self, cmd: str) -> None:
        ...
        if body:
            try:
                json_data = self.send(body, expected_response_type="json")
            except InvalidContentTypeError as err:
    ...
    async def get_host_data(self) -> None:
        ...
        try:
            ...
            json_data = []
            for command in body:
                json_data.extend(self.send([command], expected_response_type="json"))
            self.map_channels_json_response(json_data, channels)
        ...
        if self.protocol == "rtsp":
            # Cache the RTSP urls
            for channel in self._stream_channels:
                self.get_rtsp_stream_source(channel, "sub")
                self.get_rtsp_stream_source(channel, "main")
    ...
    async def _check_reolink_firmware(self) -> NewSoftwareVersion:
        ...
        if self._nvr_sw_hardware_id is None or self._nvr_sw_model_id is None:
            ...
            json_data = self.send_reolink_com(request_URL)
    ...
    async def check_new_firmware(self) -> Literal[False] | NewSoftwareVersion | str:
        ...
        try:
            latest_software_version = self._check_reolink_firmware()
        except ReolinkError as err:
            _LOGGER.debug(err)
            if new_firmware == 0:
                return False
    ...
    async def get_snapshot(self, channel: int, stream: Optional[str] = None) -> bytes | None:
        ...
        body: typings.reolink_json = [{}]
        response = self.send(body, param, expected_response_type="image/jpeg")
        if response is None or response == b"":
            ...
            self.expire_session(unsubscribe=False)
            return None
    ...
    async def _check_rtsp_url(self, url: str, channel: int, stream: str) -> bool:
        ...
        try:
            async with RTSPConnection(host=self._host, port=self._rtsp_port, username=self._username, password=self._password, logger=_LOGGER_RTSP) as rtsp_conn:
                response = rtsp_conn.send_request("DESCRIBE", url_clean)
    ...
    async def get_vod_source(
        ...
        if request_type == VodRequestType.RTMP:
            ...
            if self._rtmp_enabled is None:
                self.get_state("GetNetPort")
            if self._rtmp_enabled is False:
                self.set_net_port(enable_rtmp=True)
    ...
    async def download_vod(
        ...
        if self.is_nvr():
            ...
            filename = self._generate_NVR_download_vod(filename, start_time, end_time, channel, stream)

        param: dict[str, Any] = {"cmd": "Download", "source": filename, "output": wanted_filename}
        body = [{}]
        response = self.send(body, param, expected_response_type="application/octet-stream")
    ...
    async def set_focus(self, channel: int, focus: int) -> None:
        ...
        self.send_setting(body)
        asyncio.sleep(3)
        self.get_state(cmd="GetZoomFocus")
    ...
    async def set_whiteled(self, channel: int, state: bool | None = None, brightness: int | None = None, mode: int | str | None = None) -> None:
        ...
        self.send_setting(body, wait_before_get=3)
    ...
    async def set_spotlight(self, channel: int, enable: bool) -> None:
        ...
        if enable:
            self.set_spotlight_lighting_schedule(channel, 23, 59, 0, 0)
            self.set_whiteled(channel, enable, 100, 3)
            return

        self.set_spotlight_lighting_schedule(channel, 0, 0, 0, 0)
        self.set_whiteled(channel, enable, 100, 1)
    ...
    async def request_vod_files(
        ...
        try:
            json_data = self.send(body, {"cmd": "Search"}, expected_response_type="json")
        except InvalidContentTypeError as err:
    ...
    async def send_setting(self, body: typings.reolink_json, wait_before_get: int = 0) -> None:
        ...
        try:
            json_data = self.send(body, {"cmd": command}, expected_response_type="json")
        except InvalidContentTypeError as err:
        ...
        if command[:3] == "Set":
            ...
            if wait_before_get > 0:
                asyncio.sleep(wait_before_get)
            self.get_state(cmd=getcmd)
    ...
    async def send(
        ...
        if len_body <= MAX_CHUNK_ITEMS or expected_response_type != "json":
            return self.send_chunk(body, param, expected_response_type, retry)
        ...
        for chunk in range(0, len_body, MAX_CHUNK_ITEMS):
            ...
            response.extend(self.send_chunk(body[chunk:chunk_end], param, expected_response_type, retry))
        ...
        try:
            ...
            if expected_response_type == "image/jpeg":
                async with self._send_mutex:
                    response = self._aiohttp_session.get(url=self._url, params=param, allow_redirects=False)
                ...
                async with self._send_mutex:
                    ...
                    response = self._aiohttp_session.get(url=self._url, params=param, allow_redirects=False, timeout=dl_timeout)

                data = ""  # Response will be a file and be large, pass the response instead of reading it here.
                if response.content_type == "text/html":
                    data = response.text(encoding="utf-8")  # Error occured, read the error message
            else:
                if _LOGGER.isEnabledFor(logging.DEBUG):
                    _LOGGER.debug("%s/%s:%s::send() HTTP Request body =\n%s\n", self.nvr_name(), self._host, self._port, str(body).replace(self._password, "<password>"))

                async with self._send_mutex:
                    response = self._aiohttp_session.post(url=self._url, json=body, params=param, allow_redirects=False)

                data = response.text(encoding="utf-8")  # returns str
            ...
            if len(data) < 500 and response.content_type == "text/html":
                ...
                if login_err:
                    ...
                    self.expire_session()
                    return self.send(body, param, expected_response_type, retry)
    ...
    async def send_reolink_com(
        ...
        try:
            response = self._aiohttp_session.get(url=URL, timeout=com_timeout)
        except (aiohttp.ClientConnectorError, aiohttp.ClientOSError, aiohttp.ServerConnectionError) as err:
    ...
    async def subscription_send(self, headers, data, timeout: aiohttp.ClientTimeout | None = None, mutex: asyncio.Lock | None = None) -> str:
        ...
        try:
            async with mutex:
                response = self._aiohttp_session.post(
    ...
    async def subscribe(self, webhook_url: str | None = None, sub_type: Literal[SubType.push, SubType.long_poll] = SubType.push, retry: bool = False):
        ...
        parameters.update(self.get_digest())
        local_time = datetime.utcnow()

        xml = template.format(**parameters)

        try:
            response = self.subscription_send(headers, xml)
        except NotSupportedError as err:
            raise err
        except ReolinkError as err:
            if not retry:
                _LOGGER.debug("Reolink %s subscribe error: %s", sub_type, str(err))
                self.unsubscribe_all(sub_type)
                return self.subscribe(webhook_url, sub_type, retry=True)
        ...
        self._subscription_time_difference[sub_type] = self.calc_time_difference(local_time, remote_time)
    ...
    async def pull_point_request(self):
        ...
        try:
            response = self.subscription_send(headers, xml, timeout, mutex=self._long_poll_mutex)
        except ReolinkError as err:
        ...
        return self.ONVIF_event_callback(response, root)

    async def unsubscribe(self, sub_type: SubType = SubType.all):
        """Unsubscribe from ONVIF events."""
        if sub_type == SubType.all:
            self.unsubscribe(SubType.push)
            self.unsubscribe(SubType.long_poll)
            return
        ...
        if sub_type in self._subscription_manager_url:
            ...
            try:
                self.subscription_send(headers, xml)
            except ReolinkError as err:
    ...
    async def ONVIF_event_callback(self, data: str, root: XML.Element | None = None) -> list[int] | None:
        ...
        for message in root.iter("{http://docs.oasis-open.org/wsn/b-2}NotificationMessage"):
            ...
            if channel is None:
                ...
                if not self.get_motion_state_all_ch():
        ...
        if self._onvif_only_motion[sub_type] and any(self.ai_supported(ch) for ch in event_channels):
            ...
            if not self.get_ai_state_all_ch():
