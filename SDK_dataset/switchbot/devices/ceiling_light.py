...
class SwitchbotCeilingLight(SwitchbotBaseLight):
    ...
    async def turn_on(self) -> bool:
        """Turn device on."""
        result = self._send_command(CEILING_LIGHT_ON_KEY)
    ...
    async def turn_off(self) -> bool:
        """Turn device off."""
        result = self._send_command(CEILING_LIGHT_OFF_KEY)
        ret = self._check_command_result(result, 0, {0x01})
    ...
    async def set_brightness(self, brightness: int) -> bool:
        ...
        result = self._send_command(f"{BRIGHTNESS_KEY}{brightness:02X}0FA1")
        ret = self._check_command_result(result, 0, {0x01})
