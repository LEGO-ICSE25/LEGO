...
class SwitchbotBaseDevice:
    ...
    async def _send_command(self, key: str, retry: int | None = None) -> bytes | None:
        ...
        async with self._operation_lock:
            for attempt in range(max_attempts):
                try:
                    return self._send_command_locked(key, command)
    ...
    async def _ensure_connected(self):
        ...
        async with self._connect_lock:
            ...
            client: BleakClientWithServiceCache = establish_connection(
                BleakClientWithServiceCache,
                self._device,
                self.name(),
                self._disconnected,
            ...
            try:
                ...
                self._execute_disconnect_with_lock()
                raise
    ...
    async def _send_command_locked(self, key: str, command: bytes) -> bytes:
        ...
        try:
            ...
            asyncio.sleep(DBUS_ERROR_BACKOFF_TIME)
            _LOGGER.debug(
            ...
            self._execute_forced_disconnect()
            raise
        except BLEAK_RETRY_EXCEPTIONS as ex:
            # Disconnect so we can reset state and try again
            _LOGGER.debug(
    ...
    async def _start_notify(self) -> None:
        ...
        self._client.start_notify(self._read_char, self._notification_handler)
    ...
    async def _execute_command_locked(self, key: str, command: bytes) -> bytes:
        ...
        client.write_gatt_char(self._write_char, command, False)

        async with async_timeout.timeout(5):
            notify_msg = self._notify_future
    ...
    async def get_basic_info(self) -> dict[str, Any] | None:
        ...
        if not (_data := self._get_basic_info()):
            return None
        return {
            "battery": _data[1],
