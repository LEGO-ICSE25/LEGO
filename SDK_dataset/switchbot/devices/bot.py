...
class Switchbot(SwitchbotDeviceOverrideStateDuringConnection):
    ...
    async def turn_on(self) -> bool:
        """Turn device on."""
        result = self._send_command(ON_KEY)
    ...
    async def turn_off(self) -> bool:
        """Turn device off."""
        result = self._send_command(OFF_KEY)
        ret = self._check_command_result(result, 0, {1, 5})
    ...
    async def hand_up(self) -> bool:
        """Raise device arm."""
        result = self._send_command(UP_KEY)
        return self._check_command_result(result, 0, {1, 5})

    @update_after_operation
    async def hand_down(self) -> bool:
        """Lower device arm."""
        result = self._send_command(DOWN_KEY)
        return self._check_command_result(result, 0, {1, 5})

    @update_after_operation
    async def press(self) -> bool:
        """Press command to device."""
        result = self._send_command(PRESS_KEY)
        return self._check_command_result(result, 0, {1, 5})
    ...
    async def set_switch_mode(
        ...
        result = self._send_command(DEVICE_SET_MODE_KEY + strength_key + mode_key)
    ...
    async def set_long_press(self, duration: int = 0) -> bool:
        ...
        result = self._send_command(DEVICE_SET_EXTENDED_KEY + "08" + duration_key)
        return self._check_command_result(result, 0, {1})
