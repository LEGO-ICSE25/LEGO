...
class SwitchbotHumidifier(SwitchbotDevice):
    ...
    async def _async_set_state(self, state: bool) -> bool:
        level = self.get_target_humidity() or 128
        result = self._send_command(self._generate_command(on=state, level=level))
    ...
    async def turn_on(self) -> bool:
        """Turn device on."""
        self._async_set_state(True)

    async def turn_off(self) -> bool:
        """Turn device off."""
        self._async_set_state(False)
    ...
    async def _set_level(self, level: int) -> bool:
        """Set level."""
        result = self._send_command(self._generate_command(level=level))
        ret = self._check_command_result(result, 0, {0x01})
