...
class SwitchbotLock(SwitchbotDevice):
    ...
    async def _lock_unlock(
        ...
        if basic_data := self._get_basic_info():
    ...
    async def get_basic_info(self) -> dict[str, Any] | None:
        ...
        lock_raw_data = self._get_lock_info()
        if not lock_raw_data:
            return None

        basic_data = self._get_basic_info()
        if not basic_data:
            return None
    ...
    async def _get_lock_info(self) -> bytes | None:
        ...
        _data = self._send_command(key=COMMAND_LOCK_INFO, retry=self._retry_count)
    ...
    async def _enable_notifications(self) -> bool:
        if self._notifications_enabled:
            return True
        result = self._send_command(COMMAND_ENABLE_NOTIFICATIONS)
    ...
    async def _disable_notifications(self) -> bool:
        if not self._notifications_enabled:
            return True
        result = self._send_command(COMMAND_DISABLE_NOTIFICATIONS)
        if self._check_command_result(result, 0, COMMAND_RESULT_EXPECTED_VALUES):
    ...
    async def _send_command(
        ...
        if not encrypt:
            return super()._send_command(key[:2] + "000000" + key[2:], retry)

        result = self._ensure_encryption_initialized()
        if not result:
        ...
        result = super()._send_command(encrypted, retry)
