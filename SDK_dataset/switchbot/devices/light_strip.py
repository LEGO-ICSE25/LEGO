...
class SwitchbotLightStrip(SwitchbotSequenceBaseLight):
    ...
    async def update(self) -> None:
        ...
        result = self._send_command(STRIP_REQUEST)
        self._update_state(result)
        super().update()

    async def turn_on(self) -> bool:
        """Turn device on."""
        result = self._send_command(STRIP_ON_KEY)
        self._update_state(result)
        return self._check_command_result(result, 1, {0x80})

    async def turn_off(self) -> bool:
        """Turn device off."""
        result = self._send_command(STRIP_OFF_KEY)
        self._update_state(result)
    ...
    async def set_brightness(self, brightness: int) -> bool:
        ...
        result = self._send_command(f"{BRIGHTNESS_KEY}{brightness:02X}")
        self._update_state(result)
        return self._check_command_result(result, 1, {0x80})
