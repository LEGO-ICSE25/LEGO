...
async def async_start_scanner(  # pylint: disable=too-many-arguments
    ...
    if BLE_SCRIPT_NAME not in script_name_to_id:
        device.script_create(BLE_SCRIPT_NAME)
        script_name_to_id = _async_get_scripts_by_name(device)
    ...
    needs_putcode = False
    try:
        code_response = device.script_getcode(ble_script_id)
    except RpcCallError:
    ...
    if needs_putcode:
        ...
        # update the script
        device.script_stop(ble_script_id)
        device.script_putcode(ble_script_id, code)
...
async def async_ensure_ble_enabled(device: RpcDevice) -> bool:
    ...
    be enabled.
    """
    ble_config = device.ble_getconfig()
    if ble_config["enable"]:
        return False
    ble_enable = device.ble_setconfig(enable=True, enable_rpc=True)
    if not ble_enable["restart_required"]:
        return False
    LOGGER.info("BLE enabled, restarting device %s", device.ip_address)
    device.trigger_reboot(3500)
    return True
