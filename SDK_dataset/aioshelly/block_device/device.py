...
class BlockDevice:
    ...
    async def create(
        ...
        """Device creation."""
        options = process_ip_or_options(ip_or_options)
        instance = cls(coap_context, aiohttp_session, options)

        if initialize:
            instance.initialize()
        else:
            instance._coap_request("s")
    ...
    async def update_status(self) -> None:
        ...
        self._status = self.http_request("get", "status")

    async def update_settings(self) -> None:
        """Device update from /settings (HTTP)."""
        self._settings = self.http_request("get", "settings")

    async def update_shelly(self) -> None:
        """Device update for /shelly (HTTP)."""
        self._shelly = get_info(self.aiohttp_session, self.options.ip_address())
    ...
    async def _coap_request(self, path: str) -> asyncio.Event:
        ...
        self.coap_context.request(self.ip_address(), path)
        return event
    ...
    async def http_request(
        ...
        try:
            resp: ClientResponse = self.aiohttp_session.request(
                method,
...
class Block:
    ...
    async def set_state(self, **kwargs: Any) -> dict[str, Any]:
        ...
        return self.device.http_request(
            "get", f"{self.type}/{self.channel()}", kwargs
        )

    async def toggle(self) -> dict[str, Any]:
        """Toggle status."""
        return self.set_state(turn="off" if self.output else "on")
