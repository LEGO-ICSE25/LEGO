...
async def delete_api_key(
    ...
    response = request(session.delete, url)
...
async def delete_all_keys(
    ...
    for key in response["whitelist"]:
        if key not in api_keys:
            delete_api_key(session, host, port, key)
...
async def discovery(session: aiohttp.ClientSession) -> list[DiscoveredBridge]:
    ...
    response: list[dict[str, Any]] = request(session.get, URL_DISCOVER)
