...
class MotionEyeClient:
    ...
    async def _async_request(
        ...
        try:
            async with coro as response:
                ...
                try:
                    return_value: dict[str, Any] | None = response.json(
                        content_type=None
                    )
                    return return_value
                except (json.decoder.JSONDecodeError, UnicodeDecodeError) as exc:
                    _LOGGER.error(f"Could not JSON decode: {response.read()!r}")
    ...
    async def async_client_login(self) -> dict[str, Any] | None:
        ...
        return self._async_request("/login")
