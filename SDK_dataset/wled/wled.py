...
class WLED:
    ...
    async def connect(self) -> None:
        ...
        try:
            self._client = self.session.ws_connect(url=url, heartbeat=30)
        except (
    ...
    async def update(self, *, full_update: bool = False) -> Device:  # noqa: PLR0912
        ...
        if self._device is None or full_update:
            ...
            try:
                presets = self.request("/presets.json")
            ...
            with suppress(WLEDError):
                versions = self.get_wled_versions_from_github()
        ...
        if self._supports_presets:
            if not (presets := self.request("/presets.json")):
                msg = (
                    f"WLED device at {self.host} returned an empty API"
        ...
        if not self._supports_si_request:
            if not (info := self.request("/json/info")):
                msg = (
                    f"WLED device at {self.host} returned an empty API"
            ...
            if not (state := self.request("/json/state")):
                msg = (
        ...
        if not (state_info := self.request("/json/si")):
            msg = (
                f"WLED device at {self.host} returned an empty API"
    ...
    async def master(
        ...
        *,
        ...
        self.request("/json/state", method="POST", data=state)
    ...
    async def preset(self, preset: int | str | Preset) -> None:
        ...
        self.request("/json/state", method="POST", data={"ps": preset})
    ...
    async def playlist(self, playlist: int | str | Playlist) -> None:
        ...
        self.request("/json/state", method="POST", data={"ps": playlist})
    ...
    async def live(self, live: Live) -> None:
        ...
        """
        self.request("/json/state", method="POST", data={"lor": live.value})
    ...
    async def sync(
        ...
        *,
        ...
        self.request("/json/state", method="POST", data={"udpn": sync})
    ...
    async def nightlight(
        ...
        *,
        ...
        self.request("/json/state", method="POST", data={"nl": nightlight})
    ...
    async def upgrade(self, *, version: str | AwesomeVersion) -> None:
        ...
        try:
            async with async_timeout.timeout(
                ...
                form.add_field("file", download.read(), filename=update_file)
