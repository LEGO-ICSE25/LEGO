...
class Log(Http):
    ...
    async def async_log_clear_all(self) -> str:
        ret = self.async_command("log.cgi?action=clear")
        return ret.content.decode()
    ...
    async def async_log_find(
        ...
        token = (
            (self.async_log_find_start(start_time, end_time))
            .strip()
            .split("=")[1]
        )

        while True:
            content = self.async_log_find_next(token)
            tag, _, count = content.split("\r\n", 1)[0].partition("=")
        ...
        self.async_log_find_stop(token)
