...
class Http:
    ...
    async def _async_generate_token(self) -> None:
        ...
        try:
            try:
                resp = (self._async_command(cmd)).content.decode()
            except LoginError:
        ...
        _LOGGER.debug("%s Retrieving serial number", self)
        self._serial = pretty(
            (self._async_command("magicBox.cgi?action=getSerialNo"))
            .content.decode()
            .strip()
        )
    ...
    async def async_command(self, *args, **kwargs) -> httpx.Response:
        ...
        async with self._async_token_lock:
            if not self._async_token:
                self._async_generate_token()
        return self._async_command(*args, **kwargs)
    ...
    async def _async_magic_box(self, action: str) -> str:
        ret = self.async_command(f"magicBox.cgi?action={action}")
        return ret.content.decode()
