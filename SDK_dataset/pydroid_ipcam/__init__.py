...
class PyDroidIPCam:
    ...
    async def update(self) -> None:
        ...
        response = self._request("/status.json?show_avail=1")
        self.status_data = cast(Dict[str, Any], response.json())

        response = self._request("/sensors.json")
        self.sensor_data = cast(Dict[str, Any], response.json())
    ...
    async def change_setting(self, key: str, val: Union[str, int, bool]) -> bool:
        ...
        if isinstance(val, bool):
            ...
            payload = val
        response = self._request(f"/settings/{key}?set={payload}")
    ...
    async def set_zoom(self, zoom: int) -> bool:
        """Set the zoom level."""
        response = self._request(f"/settings/ptz?zoom={zoom}")
        return "Ok" in (response.text())
