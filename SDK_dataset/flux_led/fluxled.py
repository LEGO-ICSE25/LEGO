...
async def _async_run_commands(  # noqa: C901
    ...
    if options.getclock:
        buf_in(
            "{} [{}] {}".format(info["id"], info["ipaddr"], bulb.async_get_time())
        )
    ...
    if options.color is not None:
        ...
        bulb.async_set_preset_pattern(options.preset[0], options.preset[1])
    ...
    if options.settimer:
        ...
        timers = bulb.async_get_timers() or empty_timers
        ...
        bulb.async_set_timers(timers)

    if options.showtimers:
        show_timers = bulb.async_get_timers()
        if show_timers:
...
async def _async_process_bulb(  # noqa: C901
    ...
    bulb.async_setup(lambda *args: None)
    try:
        _async_run_commands(bulb, info, options)
    finally:
        bulb.async_stop()
...
async def async_main() -> None:  # noqa: C901
    ...
    if options.scan:
        ...
        if options.info:
            for addr in args:
                scanner.async_scan(timeout=6, address=addr)
            bulb_info_list = scanner.getBulbInfo()
        else:
            bulb_info_list = []
