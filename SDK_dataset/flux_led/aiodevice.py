...
class AIOWifiLedBulb(LEDENETDevice):
    ...
    async def _async_remote_config_setup(self) -> None:
        """Setup remote config."""
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_query_remote_config())
        try:
            async with asyncio_timeout(self.timeout):
                self._remote_config_future
    ...
    async def _async_switch_setup(self) -> None:
        """Setup a switch."""
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_power_restore_state_query())
        try:
            async with asyncio_timeout(self.timeout):
                self._power_restore_future
        except asyncio.TimeoutError:
    ...
    async def _async_send_state_query(self) -> None:
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_state_query())
    ...
    async def _async_wait_state_change(
        ...
        done, _ = asyncio.wait(futures, timeout=timeout, return_when=return_when)
    ...
    async def _async_set_power_state(
        ...
        self._async_send_msg(self._protocol.construct_state_change(state))
        _LOGGER.debug("%s: Waiting for power state response", self.ipaddr)
        if self._async_wait_state_change(
    ...
    async def async_turn_on(self) -> bool:
        """Turn on the device."""
        return self._async_set_power_locked(True)

    async def async_turn_off(self) -> bool:
        """Turn off the device."""
        return self._async_set_power_locked(False)
    ...
    async def _async_set_power_locked(self, state: bool) -> bool:
        async with self._power_state_lock:
            ...
            return self._async_set_power_state_with_retry(state)
    ...
    async def _async_set_power_state_with_retry(self, state: bool) -> bool:
        for idx in range(POWER_CHANGE_ATTEMPTS):
            ...
            if self._async_set_power_state(
    ...
    async def async_set_white_temp(
        ...
        if self.rgbw_color_temp_support(self.color_modes):
            self.async_set_levels(cold, cold, cold, warm, 0, persist=persist)
        else:
            self.async_set_levels(None, None, None, warm, cold, persist=persist)
    ...
    async def async_set_levels(
        ...
        self._async_process_levels_change(
    ...
    async def async_set_effect(
        ...
        if effect == EFFECT_RANDOM:
            self.async_set_random()
            return
        if effect == EFFECT_MUSIC:
            self.async_set_music_mode(brightness=brightness)
            return
        self.async_set_preset_pattern(
    ...
    async def async_set_music_mode(
        ...
        for idx, bytes_send in enumerate(
            ...
            if idx > 0:
                asyncio.sleep(COMMAND_SPACING_DELAY)
            self._async_send_msg(bytes_send)

    async def async_set_random(self) -> None:
        """Set levels randomly."""
        self._async_process_levels_change(*self._generate_random_levels_change())
    ...
    async def async_set_brightness(self, brightness: int) -> None:
        ...
        if effect:
            ...
            self.async_set_effect(effect, self.speed, effect_brightness)
            return
        if self.color_mode == COLOR_MODE_CCT:
            self.async_set_white_temp(self.color_temp, brightness)
            return
        if self.color_mode == COLOR_MODE_RGB:
            self.async_set_levels(*self.rgb_unscaled, brightness=brightness)
            return
        if self.color_mode == COLOR_MODE_RGBW:
            self.async_set_levels(*rgbw_brightness(self.rgbw, brightness))
            return
        if self.color_mode == COLOR_MODE_RGBWW:
            self.async_set_levels(*rgbww_brightness(self.rgbww, brightness))
            return
        if self.color_mode == COLOR_MODE_DIM:
            self.async_set_levels(w=brightness)
            return
    ...
    async def async_enable_remote_access(
        ...
        AIOBulbScanner().async_enable_remote_access(
    ...
    async def async_disable_remote_access(self) -> None:
        ...
        AIOBulbScanner().async_disable_remote_access(self.ipaddr)
        self._async_stop()

    async def async_reboot(self) -> None:
        """Reboot a device."""
        AIOBulbScanner().async_reboot(self.ipaddr)
        self._async_stop()
    ...
    async def async_set_device_config(
        ...
        async with self._device_config_lock:
            ...
            if isinstance(self._protocol, ALL_IC_PROTOCOLS):
                self._async_device_config_resync()
    ...
    async def async_unpair_remotes(self) -> None:
        ...
        self._async_send_msg(self._protocol.construct_unpair_remotes())
        self._async_send_msg(self._protocol.construct_query_remote_config())
    ...
    async def async_get_time(self) -> Optional[datetime]:
        ...
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_get_time())
        async with self._get_time_lock:
            self._get_time_future = self.loop.create_future()
            try:
                async with asyncio_timeout(self.timeout):
                    self._get_time_future
            except asyncio.TimeoutError:
    ...
    async def async_get_timers(self) -> Optional[List[LedTimer]]:
        ...
        if isinstance(self._protocol, ProtocolLEDENETOriginal):
            ...
            return led_timers
        self._async_send_msg(self._protocol.construct_get_timers())
        async with self._get_timers_lock:
            self._get_timers_future = self.loop.create_future()
            try:
                async with asyncio_timeout(self.timeout):
                    self._get_timers_future
            except asyncio.TimeoutError:
    ...
    async def async_set_timers(self, timer_list: List[LedTimer]) -> None:
        """Set the timers."""
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_set_timers(timer_list))

    async def async_set_time(self, time: Optional[datetime] = None) -> None:
        """Set the current time."""
        assert self._protocol is not None
        self._async_send_msg(self._protocol.construct_set_time(time))

    async def _async_device_config_resync(self) -> None:
        asyncio.sleep(DEVICE_CONFIG_WAIT_SECONDS)
        self._async_device_config_setup()

    async def _async_connect(self) -> None:
        """Create connection."""
        async with asyncio_timeout(self.timeout):
            _, self._aio_protocol = self.loop.create_connection(
