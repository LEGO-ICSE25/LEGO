...
class LightsController(BaseResourcesController[type[Light]]):
    ...
    async def turn_on(self, id: str, transition_time: int | None = None) -> None:
        """Turn on the light."""
        self.set_state(id, on=True, transition_time=transition_time)

    async def turn_off(self, id: str, transition_time: int | None = None) -> None:
        """Turn off the light."""
        self.set_state(id, on=False, transition_time=transition_time)
    ...
    async def set_brightness(
        ...
        self.set_state(id, on=True, brightness=brightness, transition_time=transition_time)

    async def set_color(
        self, id: str, x: float, y: float, transition_time: int | None = None
    ) -> None:
        """Set color to light. Turn on light if it's currently off."""
        self.set_state(id, on=True, color_xy=(x, y), transition_time=transition_time)
    ...
    async def set_color_temperature(
        ...
        self.set_state(id, on=True, color_temp=mirek, transition_time=transition_time)

    async def set_flash(self, id: str, short: bool = False) -> None:
        """Send Flash command to light."""
        if short:
            device = self.get_device(id)
            self._bridge.devices.set_identify(device.id)
        else:
            self.set_state(id, alert=AlertEffectType.BREATHE)
