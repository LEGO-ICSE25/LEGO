...
class ButtonController(BaseResourcesController[type[Button]]):
    ...
    async def _handle_event(self, evt_type: EventType, evt_data: dict | None) -> None:
        ...
        super()._handle_event(evt_type, evt_data)
    ...
    async def _handle_longpress_workaround(self, id: int):
        ...
        asyncio.sleep(1.5)  # time to initially wait for SHORT_RELEASE
        count = 0
        try:
            while count <= 20:  # = max 10 seconds
                ...
                self._handle_event(EventType.RESOURCE_UPDATED, btn_resource)
                asyncio.sleep(0.5)
                count += 1
        finally:
...
class CameraMotionController(BaseResourcesController[type[CameraMotion]]):
    ...
    async def set_enabled(self, id: str, enabled: bool) -> None:
        ...
        self.update(id, MotionPut(enabled=enabled))

    async def set_sensitivity(self, id: str, sensitivity: int) -> None:
        """Enable/Disable sensor."""
        self.update(id, CameraMotionPut(sensitivity=sensitivity))
...
class ContactController(BaseResourcesController[type[Contact]]):
    ...
    async def set_enabled(self, id: str, enabled: bool) -> None:
        """Enable/Disable sensor."""
        self.update(id, ContactPut(enabled=enabled))
...
class LightLevelController(BaseResourcesController[type[LightLevel]]):
    ...
    async def set_enabled(self, id: str, enabled: bool) -> None:
        """Enable/Disable sensor."""
        self.update(id, LightLevelPut(enabled=enabled))
